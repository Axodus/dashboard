...
from frontend.pages.config.rsi_bb_v1.user_inputs import user_inputs
...
from frontend.visualization.indicators import get_bbands_traces, get_rsi_traces
from frontend.visualization.signals import get_rsibb_v1_signal_traces
from frontend.visualization.utils import add_traces_to_fig

# Initialize the Streamlit page
initialize_st_page(title="RSI_BB V1", icon="📊", initial_sidebar_state="expanded")
backend_api_client = get_backend_api_client()

get_default_config_loader("rsi_bb_v1")
# User inputs
inputs = user_inputs()
st.session_state["default_config"].update(inputs)

st.write("### Visualizing RSI Bollinger Trading Signals")
...
add_traces_to_fig(fig, get_rsibb_v1_signal_traces(df=candles, bb_length=inputs["bb_length"], bb_std=inputs["bb_std"],
                                                  bb_long_threshold=inputs["bb_long_threshold"], bb_short_threshold=inputs["bb_short_threshold"],
                                                  rsi_period=inputs["rsi_period"], rsi_oversold=inputs["rsi_oversold"], rsi_overbought=inputs["rsi_overbought"]), row=1, col=1)
add_traces_to_fig(fig, get_rsi_traces(df=candles, rsi_period=inputs["rsi_period"], rsi_oversold=inputs["rsi_oversold"], rsi_overbought=inputs["rsi_overbought"]), row=2, col=1)
